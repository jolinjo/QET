	...
	ui(new Ui::GeneralConfigurationPage)
{
	...
	// 公司版精簡：不需使用者調整的設定不顯示（設定值仍保留預設）。
	// 存取元件庫的各目錄在 QGridLayout 裡：用 removeWidget 從版面移除，讓
	// grid 收合、不留空白列（純 setVisible(false) 會留下空列間距）。
	if (ui->gridLayout) {
		const QList<QWidget *> grid_removed {
...
void GeneralConfigurationPage::fillLang()
{
	...
	ui->m_lang_cb->addItem(QIcon(QStringLiteral(":/ico/24x16/tw.png")), QStringLiteral("繁體中文"), "zh_TW");
